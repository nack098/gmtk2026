Shader "Custom/TerrainTriplanar_Standard"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // URP Shadow Pragmas for standard Mesh Renderers
            ...
            Varyings Vert(Attributes input)
            {
                ...
                // 3. Compute Standard World Space Coordinates
            ...
            float4 Frag(Varyings input) : SV_Target
            {
                ...
                // 7. CEL SHADING CALCULATION (With Real Shadow Attenuation!)
                float4 shadowCoord = TransformWorldToShadowCoord(worldPos);
                Light mainLight = GetMainLight(shadowCoord);
                
                float rawNdotL = dot(N, mainLight.direction);
                
                // Remap NdotL into 0..1 range and factor in light shadow attenuation
                float NdotL = saturate(rawNdotL * 0.5 + 0.5) * mainLight.shadowAttenuation; 
        ...
        // Shadow Caster Pass (Allows the terrain mesh to cast shadows on itself and player)
        Pass
        {
            Name "ShadowCaster"
            ...
            float4 ShadowPassFrag(Varyings input) : SV_Target
            {
                return 0;
            }
            ENDHLSL
